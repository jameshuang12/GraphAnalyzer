...
class SMain():
    ...
    def percentageChange(self, clientData):
        ...
        first_call = getattr(clientData, 'close')
        sec_call = getattr(clientData, 'close')
    ...
    def investment(self, clientData, percent_change, amount):
    ...
    def rsi_function(self, clientData, period):
        rsi_values = []
        period = 14 #rsi only works in a short time span. when shown in the long range, it flattens out to 50
    ...
    def buy_or_not(self, clientData):
        ...
        """
        rsi_decision, rsi_value = self.rsi_function(clientData, len(clientData))
        adx_decision, adx_value, pos_dir, neg_dir = self.average_dir_index(clientData, len(clientData))
